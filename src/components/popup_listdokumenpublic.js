...
const PopupListDokumenPublic = ({ sertifikatId, onClose }) => {
    ...
    const fetchDokumenList = async () => {
      try {
        const response = await axios.get(
          `${config.API_URL}/public/sertifikat/${sertifikatId}/dokumen-list`
        );
        setDokumenList(response.data.data || []);
      } catch (err) {
        setError("Gagal memuat daftar dokumen publik");
  ...
  const handleViewDocument = async (dokumen) => {
    try {
      ...
      const response = await axios.get(
        `${config.API_URL}/public/dokumen-legalitas/${dokumen.id}/view`,
        {
          responseType: "blob",
        }
      );
  ...
  const handleDownloadDocument = (id, e) => {
    e.preventDefault();
    window.open(
      `${config.API_URL}/public/dokumen-legalitas/${id}/download`,
      "_blank"
    );
  };
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div className="bg-white rounded-lg p-6">
          <p>Memuat daftar dokumen publik...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col">
          <div className="p-4 border-b flex justify-between items-center">
            <h3 className="text-lg font-semibold">Daftar Dokumen Legalitas Publik</h3>
            <button
              onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {dokumenList.length === 0 ? (
              <p className="text-center py-4">Tidak ada dokumen publik</p>
            ) : (
              dokumenList.map((dokumen) => (
                ...
                >
                  ...
                  <div className="flex space-x-2 ml-4">
                    ...
                    <button
                      onClick={(e) => handleDownloadDocument(dokumen.id, e)}
...
export default PopupListDokumenPublic;