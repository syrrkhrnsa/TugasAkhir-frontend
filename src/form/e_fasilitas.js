...
const EditFasilitas = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const pemetaanData = data.pemetaan_fasilitas || {};
        const tanahData = data.tanah || {};

        setFormData({
          id_fasilitas: data.id_fasilitas,
          id_pemetaan_fasilitas: data.id_pemetaan_fasilitas,
          id_tanah: data.id_tanah,
          jenis_fasilitas: pemetaanData.jenis_fasilitas || "Tidak Bergerak",
        ...
        setExistingFiles({
          view360: data.file_360
            ? `http://127.0.0.1:8000/storage/${data.file_360}`
            : null,
          gambarFasilitas: data.file_gambar
            ? `http://127.0.0.1:8000/storage/${data.file_gambar}`
            : null,
          dokumenPdf: data.file_pdf
            ? `http://127.0.0.1:8000/storage/${data.file_pdf}`
            : null,
        });
        ...
        Swal.fire({
          title: "Error",
          text: error.response?.data?.message || "Gagal memuat data fasilitas",
          icon: "error",
        });
      } finally {
        setIsLoading(false);
  ...
  const handleFileChange = (field, e) => {
    ...
    if (
      (field === "view360" || field === "gambarFasilitas") &&
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // First update pemetaan_fasilitas
      await axios.put(
      ...
      // Then update fasilitas
      const formDataToSend = new FormData();
      formDataToSend.append("_method", "PUT");
      formDataToSend.append("catatan", formData.catatan);

      if (files.view360) formDataToSend.append("file_360", files.view360);
      if (files.gambarFasilitas)
        formDataToSend.append("file_gambar", files.gambarFasilitas);
      if (files.dokumenPdf) formDataToSend.append("file_pdf", files.dokumenPdf);
        ...
        <div className="flex-1 p-4">
          <div className="bg-white shadow-lg rounded-lg p-10 mx-auto w-[90%] max-w-3xl">
            ...
            >
              ...
              <input
                type="hidden"
                name="id_pemetaan_fasilitas"
                value={formData.id_pemetaan_fasilitas}
              />
